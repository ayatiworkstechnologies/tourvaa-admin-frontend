...
with open(itinerary_path, "r", encoding="utf-8") as f:
    itinerary_code = f.read()

# Make sure LuMapPin is imported if we want to use MapPin
if "LuMapPin as MapPin" not in itinerary_code:
    itinerary_code = itinerary_code.replace(
        '  LuSparkles as Sparkles,\n',
        '  LuSparkles as Sparkles,\n  LuMapPin as MapPin,\n'
    )

new_editor_code = """function ItineraryLongDescriptionEditor({
  value,
  onChange,
}: {
  value: string;
...
start_pos = itinerary_code.find(start_marker)
end_pos = itinerary_code.find(end_marker)

if start_pos != -1 and end_pos != -1:
    itinerary_code = itinerary_code[:start_pos] + new_editor_code + "\n\n" + itinerary_code[end_pos:]
    with open(itinerary_path, "w", encoding="utf-8") as f:
        f.write(itinerary_code)
    print("TourItineraryTab.tsx updated successfully using string slicing!")
else:
