...
export default function PublicHeader() {
  ...
  const headerLogoBg = isHome && !scrolled ? "bg-white text-indigo-600" : "bg-indigo-600 text-white";
    ...
    <div className="fixed top-0 z-50 w-full px-4 pt-4 sm:px-6 lg:px-8 transition-all duration-500">
      ...
      >
        ...
        <Link href="/" className="group flex shrink-0 items-center gap-3">
          ...
          <span className={`text-[17px] font-black tracking-tight transition-colors ${headerText}`}>Tourvaa</span>
        </Link>
            ...
            return (
              <Link
                ...
                className={`relative rounded-xl px-4 py-2 text-sm font-semibold transition-all duration-200 ${
                  active
                    ? scrolled || !isHome ? "text-indigo-600 bg-indigo-50/80" : "text-white bg-white/10"
        ...
        <div className="flex items-center gap-3">
          {!sessionLoading && isPortalUser ? (
            <div className="relative hidden sm:block" ref={dropRef}>
              ...
              >
                <span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${scrolled || !isHome ? "bg-indigo-50 text-indigo-600" : "bg-white/20 text-white"}`}>
                  {initials}
                </span>
              ...
              {dropOpen && (
                <div className="absolute right-0 mt-3 w-56 overflow-hidden rounded-2xl border border-zinc-100 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
                  ...
                  >
                    <LayoutDashboard size={16} className="text-indigo-500" /> My Dashboard
                  </Link>
                  <Link
                    href={profilePath()}
                    onClick={() => setDropOpen(false)}
              ...
              return (
                <Link
                  ...
                  className={`rounded-xl px-4 py-3 text-sm font-semibold transition-colors ${
                    active ? "bg-indigo-50 text-indigo-600" : "text-zinc-600 hover:bg-zinc-50 hover:text-zinc-950"
                  }`}
                >
                  {l.label}
                </Link>
          ...
          {!sessionLoading && isPortalUser ? (
            ...
            <div className="mt-4 flex gap-3 border-t border-zinc-100 pt-4">
              ...
              <Link
                href="/register"
                onClick={() => setOpen(false)}
                className="flex-1 rounded-xl bg-indigo-600 py-3 text-center text-sm font-bold text-white shadow-sm hover:bg-indigo-700"
              >
                Sign Up
              </Link>
            </div>
