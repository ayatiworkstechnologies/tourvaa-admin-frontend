  ...
  return (
    <section className="rounded-2xl border border-[#E9EDF3] bg-white p-6 shadow-[0_1px_4px_0_rgb(0,0,0,0.04)]">
      <div className="flex items-center gap-3">
        <span className="flex h-9 w-9 flex-none items-center justify-center rounded-xl bg-[#EDF5FF] text-[#2F9FE9]">
          <Icon size={18} />
        </span>
        <div>
          <h2 className="text-lg font-black text-[#121826]">{title}</h2>
          {description && <p className="text-xs font-medium text-[#98A2B3]">{description}</p>}
        </div>
      </div>
          ...
          >
            <div className="md:col-span-2 flex flex-wrap gap-2">
              {visibleSubcategories.length === 0 ? (
                <p className="text-sm text-[#98A2B3]">No subcategories available.</p>
              ) : (
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-full border px-3.5 py-1.5 text-xs font-bold transition-colors ${
                        active
                          ? "border-[#43A9F6] bg-[#EDF5FF] text-[#2F9FE9]"
                          : "border-[#E7EAF0] text-[#667085] hover:bg-[#F7F9FC]"
                      }`}
                    >
                      {subcategory.label}
                    </button>
