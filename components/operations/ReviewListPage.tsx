...
export default function ReviewListPage({ module, title, requiredPermission }: Props) {
  ...
  const supplierColumns: DataTableColumn<ReviewRecord>[] = [
    { key: "code", header: "ID", className: "w-24", render: (row) => row.supplier_code || row.code || row.id },
    {
      ...
      render: (row) => (
        <div>
          <p className="font-bold text-[#121826]">{row.supplier_name || row.name || "-"}</p>
          <p className="mt-0.5 text-xs text-[#667085]">{row.supplier_type || row.type || "Type not provided"}</p>
        </div>
      ),
    },
    {
      ...
      render: (row) => (
        <div className="text-sm">
          <p className="font-semibold text-[#121826]">{row.country_name || "Country missing"}</p>
          <p className="text-xs text-[#667085]">{row.city_name || "City missing"}</p>
        </div>
      ),
    },
    {
    ...
    { key: "approval_status", header: "Approval", render: (row) => <StatusBadge value={row.approval_status} /> },
    {
      ...
      header: "Activity",
      render: (row) => (
        <div className="flex gap-2 text-xs font-bold text-[#667085]">
          <span className="inline-flex items-center gap-1 rounded-full bg-[#F7F9FC] px-2 py-1"><FileText size={12} /> {row.documents?.length ?? 0}</span>
          <span className="inline-flex items-center gap-1 rounded-full bg-[#F7F9FC] px-2 py-1"><Truck size={12} /> {row.vehicles?.length ?? 0}</span>
        </div>
      ),
    },
  ];
  ...
  const agentColumns: DataTableColumn<ReviewRecord>[] = [
    { key: "code", header: "ID", className: "w-24", render: (row) => row.agent_code || row.code || row.id },
    {
      ...
      render: (row) => (
        <div>
          <p className="font-bold text-[#121826]">{row.agent_name || row.name || "-"}</p>
          <p className="mt-0.5 text-xs text-[#667085]">{row.agent_type || row.type || "Type not provided"}</p>
        </div>
      ),
    },
    {
      ...
      <div className="space-y-5">
        <section className="flex flex-col gap-4 rounded-xl border border-[#E7EAF0] bg-white p-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[#667085]">{module === "suppliers" ? "Review supplier registration, business data, documents, vehicles, approval status, and markup setup." : module === "agents" ? "Review agent registration, business data, documents, invoicing, approval status, and discount setup." : "Review operational accounts, approval status, and commercial controls."}</p>
          </div>
