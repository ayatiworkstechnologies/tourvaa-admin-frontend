...
export default function CommissionRequestTab() {
  ...
  return (
    <form onSubmit={submit} className="rounded-xl border border-[#E7EAF0] bg-white p-6 shadow-sm max-w-2xl">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="font-bold text-[#121826]">Commission Request</h2>
          <p className="mt-1 text-sm text-[#667085]">Request your preferred commission / markup. Admin will review and approve it.</p>
        </div>
        <button
          type="submit"
          disabled={saving}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block">
          <span className="text-xs font-bold uppercase text-[#667085]">Commission Type</span>
          <select
        ...
        <label className="block">
          <span className="text-xs font-bold uppercase text-[#667085]">Commission Value</span>
          <div className="relative mt-1">
            <Percent className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#98A2B3]" />
            <input
              type="number"
              min="0"
              step="0.01"
              ...
              className="w-full rounded-xl border border-[#E7EAF0] py-2.5 pl-9 pr-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
              placeholder="E.g. 10"
            />
          </div>
        </label>
      ...
      <div className="mt-5 rounded-xl bg-[#F7F9FC] p-4 text-sm">
        <p className="font-bold text-[#121826]">Current admin status: <span className="text-emerald-700">{status || "not submitted"}</span></p>
        {note && <p className="mt-1 text-[#667085]">{note}</p>}
      </div>
    </form>
  );
}
