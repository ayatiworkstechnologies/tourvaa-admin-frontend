  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm text-[#667085]">Upload the required documents to get your supplier account approved.</p>
        </div>
        {allUploaded && (
      ...
      {loading ? (
        <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="animate-pulse h-20 rounded-xl border border-[#E7EAF0] bg-white" />)}</div>
      ) : (
        <div className="space-y-4">
          {DOC_TYPES.map(({ key, label }) => {
            const doc = docMap[key];
            return (
              <div key={key} className="rounded-xl border border-[#E7EAF0] bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${doc ? "bg-emerald-50" : "bg-[#F5F7FA]"}`}>
                      <FileText size={18} className={doc ? "text-emerald-600" : "text-[#98A2B3]"} />
                    </div>
                    <div>
                      <p className="font-semibold text-[#121826]">{label}</p>
                      {doc ? (
