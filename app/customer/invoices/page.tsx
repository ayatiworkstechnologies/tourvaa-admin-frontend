...
export default function CustomerInvoicesPage() {
  ...
  const columns: DataTableColumn<Invoice>[] = [
    { key: "invoice", header: "Invoice", render: (i) => i.invoice_number || `#${i.id}`, className: "font-bold text-[#121826]" },
    { key: "booking", header: "Booking", render: (i) => i.booking_id ? <Link className="font-semibold text-[#43A9F6] hover:underline" href={`/customer/bookings/${i.booking_id}`}>{i.booking_code || `Booking #${i.booking_id}`}</Link> : "-" },
    ...
    { key: "amount", header: "Amount", render: (i) => money(i.grand_total ?? i.total_amount, i.currency), className: "text-right font-bold text-[#121826]" },
    { key: "date", header: "Date", render: (i) => dateText(i.created_at), className: "hidden text-[#667085] md:table-cell" },
  ];
