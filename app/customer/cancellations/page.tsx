...
export default function CustomerCancellationsPage() {
  ...
  const columns: DataTableColumn<Cancellation>[] = [
    { key: "booking", header: "Booking", render: (c) => <Link className="font-bold text-[#43A9F6] hover:underline" href={`/customer/bookings/${c.booking_id}`}>{c.booking_code || `Booking #${c.booking_id}`}</Link> },
    { key: "tour", header: "Tour", render: (c) => c.tour_name || "-", className: "text-[#667085]" },
    { key: "reason", header: "Reason", render: (c) => c.reason || "-", className: "hidden max-w-xs truncate text-[#667085] md:table-cell" },
    ...
    { key: "date", header: "Requested", render: (c) => dateText(c.created_at), className: "hidden text-[#667085] sm:table-cell" },
  ];
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-[#121826]">Cancellations</h1>
          <p className="mt-1 text-sm text-[#667085]">Track cancellation and refund requests.</p>
        </div>
        <Link href="/customer/bookings" className="inline-flex items-center gap-2 rounded-xl bg-[#43A9F6] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#238DD7]"><FileText size={16} /> Request from Booking</Link>
      </div>
