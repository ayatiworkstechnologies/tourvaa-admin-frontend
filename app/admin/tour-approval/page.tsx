    ...
    <ModuleWrapper title="Tour Approval" requiredPermission="tours.approve">
      <div className="space-y-5">
        <section className="flex items-center justify-between rounded-xl border border-[#E7EAF0] bg-white p-6">
          <div>
            <h2 className="text-2xl font-bold text-[#121826]">Tour Approval Queue</h2>
            <p className="mt-1 text-sm text-[#667085]">Review and approve/reject supplier tour submissions.</p>
          </div>
        ...
        {loading ? (
          <div className="space-y-4">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="animate-pulse rounded-xl border border-[#E7EAF0] bg-white p-6 h-32" />
            ))}
          </div>
        ) : versions.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-[#E7EAF0] bg-white py-20 text-center">
            <CheckCircle2 size={40} className="text-emerald-400" />
            <p className="mt-4 text-lg font-bold text-[#121826]">All caught up!</p>
            <p className="mt-1 text-sm text-[#667085]">No tours are waiting for approval.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {versions.map(v => (
              <div key={v.id} className="rounded-xl border border-[#E7EAF0] bg-white p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      <h3 className="text-lg font-bold text-[#121826]">{v.snapshot?.title || `Tour #${v.tour_id}`}</h3>
                    ...
                    {v.snapshot?.short_description && (
                      <p className="mt-2 line-clamp-2 text-sm text-[#344054]">{v.snapshot.short_description}</p>
                    )}
