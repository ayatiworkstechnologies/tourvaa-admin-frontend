...
import { ArrowLeft, Ban, CheckCircle2, Eye, FileText, Percent, ShieldHalf, Truck, XCircle } from "lucide-react";

import ActionModal from "@/components/operations/ActionModal";
import ModuleWrapper from "@/components/common/ModuleWrapper";
import Loader from "@/components/ui/Loader";
import StatusBadge from "@/components/operations/StatusBadge";
import { approveReviewRecord, getReviewRecord, partialApproveReviewRecord, rejectReviewRecord, ReviewRecord, updateCommercialValue, updateReviewRecord } from "@/lib/services/operationsService";
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {rows.map(([label, value]) => (
        <div key={label} className="rounded-lg bg-[#F7F9FC] p-4">
          ...
          <div className="mt-1 text-sm font-semibold text-[#121826]">
...
function KeyValueList({ data, empty }: { data?: Record<string, unknown> | null; empty: string }) {
  ...
  if (entries.length === 0) return <p className="rounded-lg bg-[#F7F9FC] p-4 text-sm font-semibold text-[#667085]">{empty}</p>;

  return (
    <div className="grid gap-3 md:grid-cols-2">
      {entries.map(([key, value]) => (
        <div key={key} className="rounded-lg bg-[#F7F9FC] p-4">
          <p className="text-xs font-bold uppercase text-[#98A2B3]">{titleize(key)}</p>
          <p className="mt-1 break-words text-sm font-semibold text-[#121826]">{valueText(value as DetailValue)}</p>
        </div>
      ))}
    </div>
  );
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <Link href="/admin/suppliers" className="inline-flex items-center gap-2 text-sm font-bold text-[#121826] hover:text-[#2F9FE9]">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {canPartial && <button onClick={() => setModal("partial")} className="inline-flex items-center gap-2 rounded-xl border border-[#E7EAF0] px-4 py-2.5 text-sm font-bold text-[#121826] hover:bg-[#F7F9FC]"><ShieldHalf size={16} /> Request Changes</button>}
              ...
              {canBlock && <button onClick={() => isBlocked ? void run(() => updateReviewRecord("suppliers", id, { status: "active" }), "Supplier unblocked.") : setModal("block")} className="inline-flex items-center gap-2 rounded-xl border border-[#E7EAF0] px-4 py-2.5 text-sm font-bold text-[#121826] hover:bg-[#F7F9FC]"><Ban size={16} /> {isBlocked ? "Unblock" : "Block"}</button>}
              {canCommercial && <button onClick={() => setModal("commercial")} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold ${hasCommissionRequest ? "bg-amber-500 text-white hover:bg-amber-600" : "border border-[#E7EAF0] text-[#121826] hover:bg-[#F7F9FC]"}`}><Percent size={16} /> {hasCommissionRequest ? "Approve Commission" : "Commission"}</button>}
            </div>
          </div>

          <CompletionCard record={record} />

          <DetailSection title="Supplier Profile">
            <InfoGrid rows={[
              ["Name", record.supplier_name || record.name],
              ["Code", record.supplier_code || record.code],
              ["Type", record.supplier_type || record.type],
              ...
              ["Commission / Markup", record.markup_type ? `${record.markup_type}: ${record.markup_value ?? 0}` : "-"],
            ]} />
          </DetailSection>
          ...
          <DetailSection title="Documents">
            {documents.length === 0 ? (
              <p className="rounded-lg bg-[#F7F9FC] p-4 text-sm font-semibold text-[#667085]">No supplier documents uploaded yet.</p>
            ) : (
          ...
          <DetailSection title="Vehicle List">
            {vehicles.length === 0 ? (
              <p className="rounded-lg bg-[#F7F9FC] p-4 text-sm font-semibold text-[#667085]">No vehicles added yet.</p>
            ) : (
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {vehicles.map((vehicle, index) => (
                  <div key={vehicle.id ?? index} className="rounded-xl border border-[#E7EAF0] p-4">
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <Truck size={16} className="text-[#43A9F6]" />
                        <p className="text-sm font-bold text-[#121826]">{[vehicle.make, vehicle.model].filter(Boolean).join(" ") || `Vehicle ${index + 1}`}</p>
                      </div>
        ...
        <section className="rounded-xl border border-[#E7EAF0] bg-white p-10 text-center text-[#667085]">Supplier not found.</section>
      )}
    </ModuleWrapper>
  );
}
