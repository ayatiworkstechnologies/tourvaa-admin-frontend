    ...
    <ModuleWrapper title="Discounts" requiredPermission="tours.view">
      <div className="space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-[28px] font-black tracking-tight text-[#121826]">Discounts</h2>
        ...
        <div className="flex flex-wrap gap-1 rounded-xl border border-[#E7EAF0] bg-white p-1.5">
          <button
            type="button"
            onClick={() => setScopeFilter("")}
            className={`rounded-lg px-3.5 py-2 text-sm font-semibold transition-colors ${
              scopeFilter === "" ? "bg-[#43A9F6] text-white shadow-sm" : "text-[#344054] hover:bg-[#F2F4F7]"
            }`}
          >
            All ({items.length})
          </button>
          {SCOPES.map((scope) => (
            <button
              ...
              className={`rounded-lg px-3.5 py-2 text-sm font-semibold transition-colors ${
                scopeFilter === scope.value ? "bg-[#43A9F6] text-white shadow-sm" : "text-[#344054] hover:bg-[#F2F4F7]"
              }`}
            >
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-[#E7EAF0] p-10 text-center text-sm text-[#98A2B3]">
              ...
              <div key={item.id} className="rounded-2xl border border-[#E9EDF3] bg-white p-5 shadow-[0_1px_4px_0_rgb(0,0,0,0.04)]">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <span className="mt-1.5 inline-flex items-center gap-1 rounded-full bg-[#F0F3F8] px-2.5 py-0.5 text-xs font-bold text-[#667085]">
                      <Tag size={11} /> {scopeLabel(item)}
                    </span>
                    {item.discount_code && (
                      <span className="ml-1.5 mt-1.5 inline-block rounded-full bg-[#EEF8FF] px-2.5 py-0.5 text-xs font-bold text-[#43A9F6]">
                        {item.discount_code}
                      </span>
                    )}
                    <p className="mt-1.5 flex items-center gap-1 text-sm font-semibold text-[#344054]">
                      <Percent size={13} className="text-[#98A2B3]" />
                      {item.discount_value}
                  ...
                  <div className="flex shrink-0 gap-1.5">
                    <button type="button" onClick={() => setEditing({ ...item })} className="rounded-lg border border-[#E7EAF0] p-1.5 hover:bg-[#F2F4F7]">
                      <Pencil size={13} />
                    </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
            ...
            >
              <div className="mb-5 flex items-center justify-between">
                <h3 className="text-xl font-bold text-[#121826]">{editing.id ? "Edit Discount" : "New Discount"}</h3>
                <button type="button" onClick={() => setEditing(null)} className="rounded-lg p-2 text-[#667085] hover:bg-[#F7F9FC]">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="md:col-span-2">
                  <span className="mb-1 block text-xs font-bold uppercase text-[#98A2B3]">Scope *</span>
                  <select
                ...
                {editing.discount_scope === "tour" && (
                  <label className="md:col-span-2">
                    <span className="mb-1 block text-xs font-bold uppercase text-[#98A2B3]">Tour *</span>
                ...
                {editing.discount_scope === "category" && (
                  <label className="md:col-span-2">
                    <span className="mb-1 block text-xs font-bold uppercase text-[#98A2B3]">Category *</span>
                    <select
              ...
              <div className="mt-6 flex justify-end gap-3">
                <button type="button" onClick={() => setEditing(null)} className="rounded-xl border border-[#E7EAF0] px-4 py-2 text-sm font-semibold">
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={saving}
                  className="inline-flex items-center gap-2 rounded-xl bg-[#43A9F6] px-5 py-2 text-sm font-bold text-white disabled:opacity-60"
                >
