...
export default function SupplierDashboardPage() {
  ...
  return (
    <div className="space-y-6 px-5 pb-8 md:px-9">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0B1120] via-[#1D3E64] to-[#43A9F6] p-8 text-white shadow-lg md:p-10">
      ...
      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-24 animate-pulse rounded-xl border border-[#E7EAF0] bg-white" />)}
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {stats.map(({ label, value, icon: Icon, sub }) => (
            <div key={label} className="group flex items-center justify-between rounded-3xl border border-[#E7EAF0]/60 bg-white p-6 shadow-[0_2px_12px_rgb(0,0,0,0.03)] transition-all duration-300 hover:-translate-y-1 hover:border-[#43A9F6]/30 hover:shadow-xl">
              ...
              <span className="rounded-md border border-slate-100 bg-slate-50 px-2.5 py-1 text-[10px] font-bold text-[#667085]">{sub}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        <div className="rounded-3xl border border-[#E7EAF0]/60 bg-white p-8 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="font-black text-[#121826]">Commission Summary</h2>
              <p className="mt-0.5 text-sm text-[#667085]">Gross sales minus Tourvaa commission equals your net payable.</p>
            </div>
            <Link href="/supplier/earnings" className="inline-flex items-center gap-2 rounded-xl border border-[#E7EAF0] px-4 py-2 text-sm font-bold text-[#344054] hover:bg-[#F3F8FC]">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {[
              ...
              <div key={label} className="rounded-2xl border border-[#E7EAF0] p-4">
                ...
                <p className="mt-3 text-xl font-black text-[#121826]">{value}</p>
              </div>
            ))}
          </div>
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl bg-[#F7F9FC] p-4">
              <p className="text-xs font-bold uppercase text-[#98A2B3]">Paid to date</p>
              <p className="mt-1 text-lg font-black text-[#121826]">{money(commission.paid, commission.currency)}</p>
            </div>
            <div className="rounded-2xl bg-[#F7F9FC] p-4">
              <p className="text-xs font-bold uppercase text-[#98A2B3]">Pending payout</p>
              <p className="mt-1 text-lg font-black text-[#121826]">{money(commission.pending, commission.currency)}</p>
            </div>
            <div className="rounded-2xl bg-[#F7F9FC] p-4">
              <p className="text-xs font-bold uppercase text-[#98A2B3]">Avg commission</p>
              <p className="mt-1 text-lg font-black text-[#121826]">{commission.avgRate.toFixed(2)}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-[#E7EAF0]/60 bg-white p-8 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <div className="mb-5 flex items-center justify-between">
            <h2 className="font-black text-[#121826]">Payout Status</h2>
            <Link href="/supplier/payouts" className="text-sm font-bold text-[#43A9F6] hover:underline">Manage</Link>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between rounded-2xl border border-[#E7EAF0] p-4">
              <div><p className="text-xs font-bold uppercase text-[#98A2B3]">Pending / approved</p><p className="mt-1 font-black text-[#121826]">{money(commission.pendingPayoutTotal, commission.currency)}</p></div>
              <Clock3 size={20} className="text-amber-600" />
            </div>
            <div className="flex items-center justify-between rounded-2xl border border-[#E7EAF0] p-4">
              <div><p className="text-xs font-bold uppercase text-[#98A2B3]">Released payouts</p><p className="mt-1 font-black text-[#121826]">{money(commission.paidPayoutTotal, commission.currency)}</p></div>
              <ReceiptText size={20} className="text-emerald-600" />
            </div>
            {commission.latestPayout && (
              <div className="rounded-2xl bg-[#F7F9FC] p-4">
                <p className="text-xs font-bold uppercase text-[#98A2B3]">Latest payout</p>
                <div className="mt-2 flex items-center justify-between gap-3">
                  <div>
                    <p className="font-bold text-[#121826]">{commission.latestPayout.payout_code}</p>
                    <p className="text-xs text-[#667085]">{dateText(commission.latestPayout.created_at)}</p>
                  </div>
      ...
      <div className="rounded-3xl border border-[#E7EAF0]/60 bg-white p-8 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-sm text-[#667085]">Filter tour and booking data by date range and status.</p>
          </div>
          <button type="button" onClick={() => void load()} className="inline-flex items-center gap-2 rounded-lg border border-[#D0D5DD] px-4 py-2 text-sm font-bold text-[#667085] hover:bg-[#F3F8FC]">
            <RefreshCw size={14} /> Refresh
          </button>
        </div>
        <div className="mt-4 flex flex-wrap items-end gap-4">
          <div className="flex flex-col gap-1"><label className="text-xs font-bold uppercase tracking-wide text-[#667085]">Start Date</label><input type="date" title="Start date" value={filters.start_date} onChange={(e) => setFilters((f) => ({ ...f, start_date: e.target.value }))} className="rounded-lg border border-[#D0D5DD] px-3 py-2 text-sm text-[#344054] outline-none focus:border-[#43A9F6]" /></div>
          <div className="flex flex-col gap-1"><label className="text-xs font-bold uppercase tracking-wide text-[#667085]">End Date</label><input type="date" title="End date" value={filters.end_date} onChange={(e) => setFilters((f) => ({ ...f, end_date: e.target.value }))} className="rounded-lg border border-[#D0D5DD] px-3 py-2 text-sm text-[#344054] outline-none focus:border-[#43A9F6]" /></div>
          <div className="flex flex-col gap-1"><label className="text-xs font-bold uppercase tracking-wide text-[#667085]">Booking Status</label><select title="Booking status" value={filters.status} onChange={(e) => setFilters((f) => ({ ...f, status: e.target.value }))} className="rounded-lg border border-[#D0D5DD] px-3 py-2 text-sm text-[#344054] outline-none focus:border-[#43A9F6]"><option value="">All Statuses</option><option value="confirmed">Confirmed</option><option value="ongoing">Ongoing</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></div>
          <button type="button" onClick={() => setFilters({ start_date: "", end_date: "", status: "" })} className="rounded-lg border border-[#D0D5DD] px-4 py-2 text-sm font-bold text-[#667085] hover:bg-[#F3F8FC]">Reset</button>
        </div>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-3xl border border-[#E7EAF0]/60 bg-white p-8 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <div className="mb-4 flex items-center justify-between"><h2 className="font-black text-[#121826]">Recent Bookings</h2><Link href="/supplier/bookings" className="flex items-center gap-1 text-sm font-bold text-[#43A9F6] hover:underline">View all <ArrowRight size={13} /></Link></div>
          {bookings.length === 0 ? <p className="py-6 text-center text-sm text-[#667085]">No bookings yet.</p> : <div className="space-y-3">{bookings.map((b) => <div key={b.id} className="flex items-center justify-between rounded-xl border border-[#E7EAF0] px-4 py-3"><div><p className="font-semibold text-[#121826]">{b.booking_code}</p><p className="text-xs text-[#667085]">{b.tour_name ?? "-"}</p></div><div className="flex items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusColors(b.booking_status)}`}>{b.booking_status.replaceAll("_", " ")}</span><Link href={`/supplier/bookings/${b.id}`} className="text-xs font-bold text-[#43A9F6] hover:underline">View</Link></div></div>)}</div>}
        </div>

        <div className="rounded-3xl border border-[#E7EAF0]/60 bg-white p-8 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <h2 className="mb-4 font-black text-[#121826]">Quick Actions</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            {[{ href: "/supplier/tours/create", label: "Create Tour", icon: Plus }, { href: "/supplier/tours", label: "My Tours", icon: MapPinned }, { href: "/supplier/earnings", label: "Earnings", icon: CircleDollarSign }, { href: "/supplier/bookings", label: "Bookings", icon: CalendarCheck }].map(({ href, label, icon: Icon }) => <Link key={label} href={href} className="group flex items-center gap-3 rounded-2xl border border-[#E7EAF0]/60 bg-white p-5 text-sm font-bold text-[#344054] shadow-[0_2px_8px_rgb(0,0,0,0.02)] transition-all hover:-translate-y-0.5 hover:border-[#43A9F6]/30 hover:text-[#43A9F6] hover:shadow-md"><Icon size={20} className="text-[#43A9F6]" /> {label}</Link>)}
          </div>
        </div>
      </div>
    </div>
