...
type Message = { id: number; subject: string; message: string; sender_name?: string; sender_role?: string; status?: string; created_at?: string; direction?: string };
...
export default function SupplierMessagesPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await api.get("/supplier/messages", { params: { limit: 30 } });
  ...
  async function send(e: React.FormEvent) {
    ...
    setSending(true);
    try {
      await api.post("/supplier/messages", form);
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-[#667085]">Send messages to the Tourvaa team and view your communication history.</p>
      </div>
        ...
        <div className="rounded-xl border border-[#E7EAF0] bg-white shadow-sm">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="mt-1 text-sm text-[#667085]">Use the form to contact the Tourvaa support team.</p>
            </div>
          ) : (
          ...
          <form onSubmit={send} className="space-y-3">
            <div>
              ...
              <input required value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))}
                placeholder="e.g. Payment query, Tour issue"
            ...
            <div>
              ...
              <textarea required value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))} rows={5}
                ...
                className="w-full resize-none rounded-xl border border-[#E7EAF0] px-3 py-2.5 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
            </div>
