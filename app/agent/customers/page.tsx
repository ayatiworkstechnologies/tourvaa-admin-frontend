  ...
  const columns: DataTableColumn<Customer>[] = [
    {
      ...
      render: (c) => (
        <>
          <p className="font-bold text-[#121826]">{c.full_name ?? "—"}</p>
          <p className="text-xs text-[#667085]">{c.email}</p>
        </>
      ),
    },
    { key: "phone", header: "Phone", className: "hidden text-[#667085] sm:table-cell", render: (c) => c.phone ?? "—" },
    {
      key: "bookings",
      header: "Bookings",
      className: "hidden text-center font-bold text-[#121826] md:table-cell",
      render: (c) => c.total_bookings ?? c.booking_count ?? 0,
    },
    { key: "joined", header: "Joined", className: "hidden text-[#667085] lg:table-cell", render: (c) => dateText(c.created_at) },
    {
      key: "status",
      header: "Status",
      render: (c) => (
    ...
    <div className="p-6 md:p-8">
      ...
      <div className="relative mt-6 max-w-md">
        ...
        <input
          ...
          className="w-full rounded-2xl border border-[#E7EAF0]/80 bg-white py-3 pl-11 pr-4 text-sm font-medium outline-none focus:border-[#43A9F6] focus:ring-4 focus:ring-[#43A9F6]/10 transition-all"
        />
      </div>
          ...
          actions={(c) => (
            <div className="flex justify-end gap-2">
              <Link
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-[#E7EAF0]/80 bg-white px-3 py-1.5 text-xs font-bold text-[#344054] hover:bg-[#F3F8FC] hover:text-[#43A9F6] transition-all"
              >
                <Plus size={12} /> Book
              </Link>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-[#E7EAF0] bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#E7EAF0] px-6 py-4">
              <h2 className="text-base font-bold text-[#121826]">New Customer</h2>
            ...
            <form onSubmit={createCustomer} className="space-y-4 px-6 py-5">
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold uppercase tracking-wide text-[#667085]">First name *</label>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold uppercase tracking-wide text-[#667085]">Last name</label>
                  <input value={form.last_name} onChange={field("last_name")} placeholder="Smith"
                    className="rounded-xl border border-[#E7EAF0] px-3 py-2.5 text-sm outline-none focus:border-[#43A9F6]" />
                </div>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold uppercase tracking-wide text-[#667085]">Email *</label>
                <input required type="email" value={form.email} onChange={field("email")} placeholder="jane@example.com"
                  className="rounded-xl border border-[#E7EAF0] px-3 py-2.5 text-sm outline-none focus:border-[#43A9F6]" />
              </div>
              <div className="flex gap-3">
                <div className="flex flex-col gap-1.5 w-24">
                  <label className="text-xs font-bold uppercase tracking-wide text-[#667085]">Code</label>
                  <input value={form.phone_country_code} onChange={field("phone_country_code")} placeholder="+1"
                    className="rounded-xl border border-[#E7EAF0] px-3 py-2.5 text-sm outline-none focus:border-[#43A9F6]" />
                </div>
                <div className="flex flex-col gap-1.5 flex-1">
                  <label className="text-xs font-bold uppercase tracking-wide text-[#667085]">Phone</label>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={saving}
                  className="flex-1 rounded-xl bg-[#43A9F6] py-2.5 text-sm font-bold text-white hover:bg-[#2F9FE9] disabled:opacity-60 transition-all">
