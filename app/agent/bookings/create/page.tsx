      ...
      <form onSubmit={handleSubmit} className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-[#E7EAF0] bg-white p-6 shadow-sm">
            ...
            <p className="mt-1 text-xs text-[#667085]">Search an existing customer or create a new one.</p>
              ...
              <button
                ...
                className={`flex-1 rounded-lg border py-2 text-sm font-bold transition ${
                  customerMode === "search"
                    ...
                    : "border-[#E7EAF0] text-[#667085] hover:bg-[#F5F7FA]"
                }`}
              >
                Search existing
              </button>
            ...
            {customerMode === "search" ? (
                ...
                {selectedCustomer ? (
                  <div className="flex items-center justify-between rounded-xl border border-orange-300 bg-orange-50 p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-[#121826]">{selectedCustomer.name ?? selectedCustomer.full_name}</p>
                    ...
                    <button
                      ...
                      className="rounded-lg p-1.5 text-[#667085] hover:bg-white hover:text-rose-600"
                    >
                      <X size={15} />
                    </button>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-[#E7EAF0] py-2.5 pl-9 pr-4 text-sm outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-50"
                    />
                    {showCustomerDropdown && (customerLoading || customerResults.length > 0) && (
                      <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-xl border border-[#E7EAF0] bg-white shadow-lg">
                        {customerLoading ? (
                          <div className="flex items-center gap-2 px-4 py-3 text-sm text-[#667085]">
                            <Loader2 size={14} className="animate-spin" /> Searchingâ€¦
                          </div>
                        ) : (
                          customerResults.map((c) => (
                            <button
                              ...
                              className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm hover:bg-[#F5F7FA]"
                            >
                              <User size={14} className="text-[#667085]" />
                              <div>
                                <p className="font-bold text-[#121826]">{c.name ?? c.full_name}</p>
                                <p className="text-xs text-[#667085]">{c.email}</p>
                              </div>
                            </button>
                          ))
                        )}
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  <span className="text-xs font-bold uppercase text-[#667085]">Full Name *</span>
                  <input
                    value={newCustomer.full_name}
                    onChange={(e) => setNewCustomer((c) => ({ ...c, full_name: e.target.value }))}
                    placeholder="Customer full name"
                    className="mt-1 w-full rounded-xl border border-[#E7EAF0] px-4 py-2.5 text-sm outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-50"
                  />
                </label>
                <label className="block">
                  <span className="text-xs font-bold uppercase text-[#667085]">Email *</span>
                  <input
                    type="email"
          ...
          {/* Tour Section */}
          <div className="rounded-xl border border-[#E7EAF0] bg-white p-6 shadow-sm">
            <h2 className="text-base font-black text-[#121826]">Tour</h2>
            <p className="mt-1 text-xs text-[#667085]">Search and select a tour to book.</p>
              ...
              {selectedTour ? (
                <div className="flex items-center justify-between rounded-xl border border-orange-300 bg-orange-50 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-[#121826]">{selectedTour.title}</p>
                ...
                <div className="relative">
                  ...
                  {showTourDropdown && (tourLoading || tourResults.length > 0) && (
                    <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-xl border border-[#E7EAF0] bg-white shadow-lg">
                      {tourLoading ? (
                        <div className="flex items-center gap-2 px-4 py-3 text-sm text-[#667085]">
                          <Loader2 size={14} className="animate-spin" /> Searching toursâ€¦
                        </div>
                      ) : (
                        tourResults.map((t) => (
                          <button
                          ...
                          >
                            ...
                            <div>
                              <p className="font-bold text-[#121826]">{t.title}</p>
                              <p className="text-xs text-[#667085]">
          ...
          {/* Travel Details */}
          <div className="rounded-xl border border-[#E7EAF0] bg-white p-6 shadow-sm">
            <h2 className="text-base font-black text-[#121826]">Travel Details</h2>
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              <label className="block sm:col-span-3">
                <span className="text-xs font-bold uppercase text-[#667085]">Travel Date *</span>
                <input
                  type="date"
                  value={travelDate}
              ...
              <label className="block">
                <span className="text-xs font-bold uppercase text-[#667085]">Adults *</span>
                <div className="relative mt-1">
                  <input
                    ...
                    className="w-full rounded-xl border border-[#E7EAF0] px-4 py-2.5 text-sm outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-50"
                  />
                  <ChevronDown size={14} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[#98A2B3]" />
                </div>
              </label>
              <label className="block">
                <span className="text-xs font-bold uppercase text-[#667085]">Children</span>
                <input
                  type="number"
                  min={0}
                  value={children}
              ...
              <div className="flex items-end pb-1">
                <p className="text-sm text-[#667085]">
                  <span className="font-bold text-[#121826]">{totalPax}</span> total pax
                </p>
              </div>
              <label className="block sm:col-span-3">
                ...
                <textarea
                  ...
                  className="mt-1 w-full resize-none rounded-xl border border-[#E7EAF0] px-4 py-2.5 text-sm outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-50"
                />
              </label>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="sticky top-6 space-y-4">
            <div className="rounded-xl border border-[#E7EAF0] bg-white p-6 shadow-sm">
              <h2 className="text-base font-black text-[#121826]">Price Preview</h2>
              {selectedTour && unitPrice > 0 ? (
                <div className="mt-4 space-y-3">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-[#667085]">Tour</span>
                    <span className="font-bold text-[#121826] line-clamp-1 max-w-[160px] text-right">
                      {selectedTour.title}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-[#667085]">Unit price</span>
                    <span className="font-bold text-[#121826]">{money(unitPrice, currency)}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-[#667085]">Adults Ã— {adults}</span>
                    <span className="font-bold text-[#121826]">{money(unitPrice * adults, currency)}</span>
                  </div>
                  <div className="border-t border-[#E7EAF0] pt-3">
                    <div className="flex items-center justify-between">
                      <span className="font-black text-[#121826]">Estimated Total</span>
                      <span className="text-xl font-black text-orange-600">{money(estimatedTotal, currency)}</span>
                    </div>
                    <p className="mt-1 text-xs text-[#667085]">Final pricing confirmed at checkout.</p>
                  </div>
                </div>
              ) : (
                <div className="mt-4 rounded-xl border border-dashed border-[#E7EAF0] p-4 text-center text-sm text-[#667085]">
            ...
            >
              {submitting ? (
                <><Loader2 size={16} className="animate-spin" /> Creating bookingâ€¦</>
              ) : (
            ...
            <Link
              href="/agent/bookings"
              className="block w-full rounded-xl border border-[#E7EAF0] bg-white py-3 text-center text-sm font-bold text-[#667085] transition hover:bg-[#F5F7FA]"
            >
              Cancel
            </Link>
          </div>
