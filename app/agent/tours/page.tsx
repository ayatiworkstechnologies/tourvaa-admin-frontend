...
function TourSkeleton() {
  return (
    <div className="animate-pulse overflow-hidden rounded-xl border border-[#E7EAF0] bg-white shadow-sm">
      <div className="h-44 bg-[#E7EAF0]" />
      <div className="p-4 space-y-2">
        <div className="h-4 w-3/4 rounded bg-[#E7EAF0]" />
        <div className="h-3 w-1/2 rounded bg-[#E7EAF0]" />
        <div className="h-3 w-1/3 rounded bg-[#E7EAF0]" />
        <div className="mt-3 h-9 rounded-lg bg-[#E7EAF0]" />
      </div>
    </div>
  );
}
    ...
    <div className="p-6 md:p-8">
      ...
      <form onSubmit={handleSearch} className="mt-6 flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-[#E7EAF0] bg-white py-2.5 pl-9 pr-4 text-sm outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-50"
          />
        </div>
        <button
          type="submit"
      ...
      {loading && page === 1 ? (
        ...
        <div className="mt-10 flex flex-col items-center justify-center rounded-xl border border-dashed border-[#D9DEE8] py-16 text-center">
          <MapPin size={36} className="text-[#98A2B3]" />
          <p className="mt-3 font-bold text-[#121826]">No tours found</p>
          <p className="mt-1 text-sm text-[#667085]">Try a different search term or clear the filter.</p>
          {query && (
            <button
              type="button"
              onClick={() => { setSearch(""); setQuery(""); setPage(1); }}
              className="mt-4 rounded-lg border border-[#E7EAF0] px-4 py-2 text-sm font-bold text-[#344054] hover:bg-[#F5F7FA]"
            >
              Clear search
            </button>
          )}
        ...
        <>
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {tours.map((tour) => (
              <div
                key={tour.id}
                className="group overflow-hidden rounded-xl border border-[#E7EAF0] bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
              >
                {/* Thumbnail */}
                <div className="relative h-44 overflow-hidden bg-orange-50">
                  ...
                  {(tour.category_name || tour.category) && (
                    <span className="absolute left-3 top-3 rounded-full bg-white/90 px-2.5 py-0.5 text-xs font-bold text-[#121826] shadow-sm">
                      {tour.category_name ?? tour.category}
                    </span>
                  )}
                </div>
                ...
                <div className="p-4">
                  <h3 className="line-clamp-2 font-bold text-[#121826]">{tour.title}</h3>
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-xs text-[#667085]">From</p>
          ...
          {hasMore && (
            <div className="mt-8 flex justify-center">
              <button
                ...
                disabled={loading}
                className="inline-flex items-center gap-2 rounded-xl border border-[#E7EAF0] bg-white px-6 py-2.5 text-sm font-bold text-[#344054] shadow-sm transition hover:bg-[#F5F7FA] disabled:cursor-wait disabled:opacity-60"
              >
