...
export default function AgentInvoicesPage() {
  ...
  const columns: DataTableColumn<Invoice>[] = [
    { key: "invoice_number", header: "Invoice #", className: "font-bold text-[#121826]", render: (inv) => inv.invoice_number ?? `INV-${inv.id}` },
    { key: "booking", header: "Booking", className: "hidden text-[#667085] sm:table-cell", render: (inv) => inv.booking_code ?? "—" },
    { key: "customer", header: "Customer", className: "hidden text-[#667085] md:table-cell", render: (inv) => inv.customer_name ?? "—" },
    { key: "date", header: "Date", className: "hidden text-[#667085] lg:table-cell", render: (inv) => dateText(inv.issued_at) },
    { key: "amount", header: "Amount", className: "text-right font-bold text-[#121826]", render: (inv) => money(inv.amount, inv.currency) },
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      {/* Table */}
      <div className="mt-6 rounded-xl border border-[#E7EAF0] bg-white shadow-sm p-0">
        <DataTable
          ariaLabel="Invoices"
          columns={columns}
          rows={invoices}
