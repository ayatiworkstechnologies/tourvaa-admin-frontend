    ...
    <div className="p-6 md:p-8">
      <div className="relative mb-6 overflow-hidden rounded-3xl bg-linear-to-br from-[var(--portal-hero-from)] to-[var(--portal-hero-to)] p-7 text-white shadow-xl shadow-blue-200/40 md:p-9">
        ...
        <div className="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><h1 className="text-3xl font-black leading-tight tracking-tight md:text-4xl">Messages</h1><p className="mt-2 max-w-md text-sm font-medium text-blue-100">Contact Tourvaa operations and track your sent requests.</p></div>
          <button type="button" onClick={() => { setShowCompose((value) => !value); setSent(false); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-dash-brand-dark shadow-sm hover:bg-[var(--portal-soft)]"><Plus size={16} /> New Message</button>
        </div>
      </div>
      ...
      {loading ? <div className="space-y-3">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-28 animate-pulse rounded-2xl border border-dash-border bg-white" />)}</div> : messages.length === 0 ? <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-dash-border bg-white py-16 text-center"><MessageSquare size={34} className="text-blue-300" /><h2 className="mt-4 font-bold text-dash-text">No messages yet</h2><p className="mt-1 text-sm text-dash-muted">Your messages to Tourvaa operations will appear here.</p></div> : <div className="space-y-3">{messages.map((item) => <article key={item.id} className="rounded-2xl border border-dash-border bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--portal-soft)] text-dash-brand"><Mail size={18} /></div><div><h2 className="font-bold text-dash-text">{item.subject || "Agent Support Request"}</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-dash-body">{item.message}</p></div></div><div className="shrink-0 text-right"><span className="rounded-full bg-[var(--portal-soft)] px-2.5 py-1 text-xs font-bold capitalize text-dash-brand-dark">{item.email_status || "submitted"}</span><p className="mt-2 text-xs text-dash-subtle">{dateTime(item.created_at)}</p></div></div></article>)}</div>}
    </div>
  );
}
