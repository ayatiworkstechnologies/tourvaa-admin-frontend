    ...
    <div className="p-6 md:p-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
        <div className="rounded-xl border border-dash-border bg-white p-6 shadow-sm">
          ...
          <p className="mt-4 font-bold text-dash-text">{form.full_name || user?.name}</p>
            ...
            {([
              ...
              <label key={key} className="block">
                ...
                <input value={form[key]} onChange={e => set(key, e.target.value)} placeholder={ph}
                  className="mt-1 w-full rounded-xl border border-dash-border px-3 py-2.5 text-sm outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100" />
              </label>
            ))}
            <label className="block sm:col-span-2">
              <span className="text-xs font-bold uppercase text-dash-muted">Bio / Description</span>
              <textarea value={form.bio} onChange={e => set("bio", e.target.value)} rows={3} placeholder="Tell us about yourself and how you promote tours..."
                className="mt-1 w-full resize-none rounded-xl border border-dash-border px-3 py-2.5 text-sm outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100" />
            </label>
          </div>
