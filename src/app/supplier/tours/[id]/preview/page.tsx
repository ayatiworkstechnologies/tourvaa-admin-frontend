  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6 flex items-center gap-3">
        <button onClick={() => router.back()} className="flex items-center gap-2 rounded-xl border border-dash-border px-3 py-2 text-sm font-bold hover:bg-dash-bg-muted"><ArrowLeft size={14} /> Back</button>
        <div><h1 className="text-xl font-black text-dash-text">{tour.title}</h1><p className="text-xs text-dash-muted">Preview — how customers see this tour</p></div>
      ...
      <div className="overflow-hidden rounded-2xl border border-dash-border bg-white shadow-sm">
        ...
        <div className="p-6">
          ...
          {tour.price && <div className="mb-4"><span className="text-2xl font-black text-emerald-600">{tour.currency || "AED"} {Number(tour.price).toLocaleString()}</span><span className="ml-2 text-sm text-dash-muted">per person</span></div>}
          <div className="mb-4 flex gap-2 border-b border-dash-border">
            {(["overview","inclusions"] as const).map(t => <button key={t} onClick={() => setTab(t)} className={`px-4 py-2.5 text-sm font-bold capitalize ${tab === t ? "border-b-2 border-emerald-600 text-emerald-700" : "text-dash-muted"}`}>{t}</button>)}
          </div>
          {tab === "overview" && <div className="text-sm text-dash-body">{tour.description ? <p>{tour.description}</p> : <p className="text-dash-subtle">No description yet.</p>}{tour.highlights && tour.highlights.length > 0 && <div className="mt-4"><h3 className="mb-2 font-bold">Highlights</h3><ul className="space-y-1">{tour.highlights.map((h, i) => <li key={i} className="flex gap-2"><Check size={14} className="mt-0.5 shrink-0 text-emerald-500" />{h}</li>)}</ul></div>}</div>}
