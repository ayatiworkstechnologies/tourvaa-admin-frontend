  ...
  if (loadingTour) {
    return (
      <div className="p-6 md:p-8">
        <div className="mb-4 h-5 w-24 animate-pulse rounded bg-dash-border" />
        <div className="mb-6 h-8 w-64 animate-pulse rounded-lg bg-dash-border" />
        <div className="h-96 animate-pulse rounded-xl border border-dash-border bg-white" />
      </div>
    );
  }
    ...
    <div className="p-6 md:p-8">
      ...
      <div className="mb-6">
        ...
        <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-xl font-black text-dash-text line-clamp-2">
              {tour?.title ?? "Edit Tour"}
            </h1>
            {tour?.tour_code && (
              <p className="mt-0.5 text-xs text-dash-subtle">{tour.tour_code}</p>
            )}
          </div>
      ...
      <div className="mb-5 overflow-x-auto">
        <div className="flex min-w-max gap-1 rounded-xl border border-dash-border bg-white p-1.5">
          {TABS.map((tab, i) => (
            <button
              key={tab}
              type="button"
              ...
              className={`rounded-lg px-3.5 py-2 text-xs font-bold whitespace-nowrap transition ${
                activeTab === i
                  ...
                  : "text-dash-muted hover:bg-dash-bg-muted hover:text-dash-body"
              }`}
            >
              {tab}
            </button>
      ...
      <div className="rounded-xl border border-dash-border bg-white p-6 shadow-sm">
        ...
        {activeTab === 12 && (
          <div className="py-12 text-center text-sm text-dash-muted">
