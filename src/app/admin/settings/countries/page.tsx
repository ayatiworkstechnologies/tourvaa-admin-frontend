...
function CrudTab({ title, endpoint, fields, columns, extraParams, canCreate, canEdit }: TabProps) {
  ...
  const allColumns: DataTableColumn<Row>[] = [
    ...
    ...(canEdit ? [{
      ...
      header: "",
      render: (r: Row) => (
        <button type="button" title="Edit" onClick={() => openEdit(r)} className="rounded-lg p-1.5 text-dash-brand hover:bg-[#F3F8FC]">
          <Edit size={15} />
        </button>
      ),
    }] : []),
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <input
          ...
          className="w-72 rounded-xl border border-dash-border bg-white px-3 py-2 text-sm outline-none focus:border-dash-brand"
        />
        {canCreate && (
          <button type="button" onClick={openAdd}
            className="inline-flex items-center gap-2 rounded-xl bg-dash-brand px-4 py-2 text-sm font-bold text-white hover:bg-dash-brand-dark">
        ...
        <div className="space-y-4">
          {fields.map((f) => (
            <div key={f.name} className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-dash-body">{f.label}</label>
              {f.type === "select" ? (
                <select
                  title={f.label}
