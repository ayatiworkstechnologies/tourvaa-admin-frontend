  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-dash-text">Audit Logs</h1>
          <p className="mt-1 text-sm text-dash-muted">System-wide record of all admin actions and changes.</p>
        </div>
        <button onClick={() => void load()} className="inline-flex items-center gap-2 rounded-xl border border-dash-border px-4 py-2.5 text-sm font-bold hover:bg-dash-bg-muted">
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        <form onSubmit={handleSearch} className="flex gap-2">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by user or action"
            className="rounded-xl border border-dash-border px-3 py-2 text-sm outline-none focus:border-[#0284C7] w-64" />
