  ...
  return (
    <ModuleWrapper title="Supplier Payouts" requiredPermission="supplier_ledger.view">
      ...
      <div className="mb-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "Filtered Total", value: money(summary.totalAmount, summary.currency), icon: WalletCards, tone: "text-dash-brand bg-[#E7F5FF]" },
      ...
      <div className="rounded-xl border border-dash-border bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-dash-border px-5 py-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-dash-subtle">Showing {visiblePayouts.length} matching records</p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <label className="relative block sm:w-64">
              ...
              <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search payouts"
                className="w-full rounded-xl border border-dash-border bg-white py-2 pl-9 pr-3 text-sm outline-none focus:border-dash-brand" />
            </label>
