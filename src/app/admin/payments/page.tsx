...
function StatusBadge({ status }: { status: string }) {
  ...
  const cls =
    ...
    "bg-[#F2F4F7] text-[#475467] border border-dash-border";
  return (
...
function ActionModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form onSubmit={submit} className="w-full max-w-sm rounded-2xl border border-dash-border bg-white p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-base font-black text-dash-text">{titles[action]}</h3>
          <button type="button" onClick={onClose} aria-label="Close" disabled={saving}
            className="rounded-xl p-1.5 text-dash-muted hover:bg-dash-bg">
            <X size={16} />
          </button>
        </div>
        ...
        {(action === "void" || action === "refund") && (
          <label className="mb-3 block">
            <span className="mb-1 block text-xs font-bold uppercase text-dash-muted">Reason (optional)</span>
            <input
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button type="button" onClick={onClose} disabled={saving}
            className="rounded-xl border border-dash-border px-4 py-2 text-sm font-bold text-dash-muted hover:bg-dash-bg disabled:opacity-60">
            Cancel
          </button>
        </div>
      </form>
  ...
  return (
    <>
      <button ref={buttonRef} type="button" onClick={() => setOpen(v => !v)} disabled={saving}
        className="rounded-lg border border-dash-border px-2 py-1 text-[10px] font-bold text-dash-muted hover:border-[#B0B8C9] hover:bg-dash-bg disabled:opacity-50">
      ...
      {open &&
        ...
        createPortal(
          <div
            ...
            className="z-50 w-48 rounded-xl border border-dash-border bg-white py-1 shadow-lg"
          >
            {STATUSES.map(s => (
              <button key={s} type="button" onClick={() => choose(s)}
                className="w-full px-3 py-1.5 text-left text-xs font-bold capitalize text-dash-body hover:bg-dash-bg">
                {s.replaceAll("_", " ")}
              </button>
            ))}
          </div>,
...
export default function PaymentsPage() {
  ...
  const statCards = useMemo(
    () => [
      { label: "Total Payments", value: stats.total, icon: Wallet, accent: "text-dash-brand-hover bg-[#EDF5FF]" },
  ...
  const columns: DataTableColumn<Payment>[] = [
    { key: "payment_code", header: "Code", className: "font-bold text-dash-text" },
    {
      ...
      render: p => (
        <div>
          <p className="font-semibold text-dash-text">{p.customer_name || `Customer #${p.customer_id}`}</p>
          {p.customer_email && <p className="text-xs text-dash-subtle">{p.customer_email}</p>}
        </div>
      ),
    },
    {
      ...
      header: "Booking",
      render: p => (
        <Link href={`/admin/bookings/${p.booking_id}`} className="font-semibold text-dash-brand-hover hover:underline">
  ...
  return (
    <ModuleWrapper title="Payments" requiredPermission="payments.view">
      <div className="space-y-6">
        <div>
          <h1 className="text-[28px] font-black tracking-tight text-dash-text">Payments</h1>
