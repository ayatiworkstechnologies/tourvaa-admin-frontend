...
export default function UserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function activate() {
    ...
    const isReactivation = Boolean(user?.deactivated_at) || ["INACTIVE", "SUSPENDED", "LOCKED"].includes(user?.account_status || "");
    try {
      if (isReactivation) {
        await api.post(`/users/${id}/reactivate`);
        setMessage("Account reactivated. The user can sign in again.");
      } else {
        await api.post(`/users/${id}/activate`, { role_id: user?.role_id });
  ...
  return <DashboardLayout title="User details" menus={dashboard.menus} user={dashboard.user}><div className="space-y-6">
    ...
    <section className="rounded-2xl border border-dash-border bg-white p-6">
      ...
      <div className="mt-6 border-t border-dash-border pt-6">
        {user.account_status === "ACTIVE" ? <div className="max-w-xl"><label className="text-xs font-bold text-dash-text">Reason for deactivation</label><textarea value={reason} onChange={(e) => setReason(e.target.value)} className="mt-2 w-full rounded-xl border border-dash-border p-3 text-sm" rows={3} /><button onClick={() => void deactivate()} disabled={saving} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 text-sm font-bold text-white"><Power size={15} />Deactivate account</button></div> : <div><button onClick={() => void activate()} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50"><Check size={16} />{user.deactivated_at ? "Reactivate account" : "Activate account"}</button>{(!user.password_created || !user.email_verified) && <p className="mt-2 text-xs text-amber-700">Activation requires a created password and verified email. If either is missing, the server will explain the required next step.</p>}</div>}
      </div>
    </section>
