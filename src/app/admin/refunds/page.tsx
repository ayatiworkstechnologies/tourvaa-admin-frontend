...
export default function RefundsPage() {
  ...
  const reqColumns: DataTableColumn<CancellationRequest>[] = [
    ...
    },
    {
      key: "reason",
      header: "Reason",
      className: "max-w-xs text-dash-body",
    ...
    <ModuleWrapper title="Refunds" requiredPermission="bookings.manage">
      <div className="space-y-5">
        ...
        <section className="flex items-center justify-between rounded-xl border border-dash-border bg-white p-6">
          <div>
            <h2 className="text-2xl font-bold text-dash-text">Refund Management</h2>
            <p className="mt-1 text-sm text-dash-muted">Handle cancellation requests and configure refund rules.</p>
          </div>
          <RefreshCcw size={20} className="text-[#0284C7]" />
        </section>

        {/* Tabs */}
        <div className="flex gap-1 rounded-xl border border-dash-border bg-white p-1.5">
          {TABS.map(tab => (
            <button
              key={tab}
              type="button"
              ...
              className={`flex-1 rounded-lg px-4 py-2.5 text-sm font-bold transition ${
                activeTab === tab
                  ...
                  : "text-dash-muted hover:bg-[#F3F8FC] hover:text-[#0284C7]"
              }`}
            >
              {tab}
            </button>
        ...
        {activeTab === "Cancellation Requests" && (
          <div className="rounded-xl border border-dash-border bg-white">
            <div className="p-0">
              <DataTable
                ...
                actions={(req) => (
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {req.gateway_refund_id && (
                      <span className="text-xs text-dash-subtle">Ref: {req.gateway_refund_id}</span>
                    )}
                  </div>
                )}
                        ...
                        <td colSpan={6} className="px-6 py-4">
                          <div className="space-y-3">
                            ...
                            <div className="grid gap-3 sm:grid-cols-3">
                              <div>
                                <label className="mb-1 block text-xs font-semibold text-dash-muted">Refund %</label>
                                <input
                                  type="number"
                                  min={0}
                                  max={100}
                              ...
                              <div>
                                <label className="mb-1 block text-xs font-semibold text-dash-muted">Refund Amount (override)</label>
                                <input
                                  type="text"
                              ...
                              <div>
                                <label className="mb-1 block text-xs font-semibold text-dash-muted">Admin Notes</label>
                                <input
                                  type="text"
          ...
          <div className="space-y-4">
            ...
            {showRuleForm && (
              <div className="rounded-xl border border-dash-border bg-white p-6">
                <h3 className="mb-4 text-base font-bold text-dash-text">New Refund Rule</h3>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <div>
                    <label className="mb-1 block text-xs font-bold uppercase text-dash-muted">Tour ID (leave blank for global)</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-bold uppercase text-dash-muted">Refund % *</label>
                    <input
                      type="number"
                      min={0}
                      max={100}
                  ...
                  <div className="sm:col-span-2">
                    <label className="mb-1 block text-xs font-bold uppercase text-dash-muted">Description</label>
                    <input
                      type="text"
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <button
                    ...
                    className="rounded-xl border border-dash-border px-4 py-2.5 text-sm font-semibold text-dash-muted hover:bg-dash-bg"
                  >
                    Cancel
                  </button>
                </div>
