...
function AdminDashboardContent({ user }: { user: { name: string; role: { name: string } } }) {
  ...
  return (
    <div className="space-y-6 pb-10 font-sans">
      ...
      {/* breadcrumb */}
      <nav className="flex items-center gap-1.5 text-[13px] font-semibold text-dash-muted">
        <Link href="/admin/dashboard" className="flex items-center gap-1 hover:text-dash-brand transition-colors">
          <Home size={14} /> Home
        </Link>
        <ChevronRight size={13} className="text-[#D0D5DD]" />
        <span className="text-dash-text">Dashboard</span>
      </nav>
        ...
        {stats.map((stat, idx) => (
          <div key={idx} className="group flex cursor-pointer items-center gap-5 rounded-3xl border border-dash-border/60 bg-white p-6 shadow-[0_2px_12px_rgb(0,0,0,0.03)] transition-all duration-300 hover:-translate-y-1 hover:border-dash-brand/30 hover:shadow-xl">
            ...
            <div className="flex flex-1 flex-col justify-between self-stretch">
              <span className="text-xs font-bold uppercase tracking-wider text-dash-muted">{stat.label}</span>
              <div className="mt-2 flex items-end justify-between">
                <span className="text-2xl font-black text-dash-text">{stat.value}</span>
                <span className="rounded-md border border-slate-100 bg-slate-50 px-2.5 py-1 text-[10px] font-bold text-dash-muted">
                  {stat.sub}
                </span>
              </div>
            </div>
      ...
      <div className="rounded-3xl border border-dash-border/60 bg-white p-6 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
        <div className="mb-5">
          <h2 className="text-base font-black text-dash-text">Dashboard Filters</h2>
          <p className="mt-0.5 text-xs font-semibold text-dash-muted">Filter analytics by date range or country, then press Apply.</p>
        </div>
        <div className="grid grid-cols-1 items-end gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full appearance-none rounded-xl border border-dash-border bg-white px-4 py-2.5 text-sm font-semibold text-dash-text outline-none focus:border-dash-brand focus:ring-1 focus:ring-dash-brand"
            >
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex h-10.5 flex-1 items-center justify-center gap-2 rounded-xl bg-dash-brand px-4 text-sm font-bold text-white hover:bg-[#2196e0] transition-colors"
            >
        ...
        {(activeFilters.start || activeFilters.end || activeFilters.country) && (
          <p className="mt-3 text-[11px] font-semibold text-dash-brand">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-3xl border border-dash-border/60 bg-white p-6 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <div className="mb-4 flex items-center gap-2 text-dash-text">
            <BarChart3 size={18} className="text-dash-brand" />
        ...
        <div className="rounded-3xl border border-dash-border/60 bg-white p-6 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <div className="mb-4 flex items-center gap-2 text-dash-text">
            <CreditCard size={18} className="text-dash-brand" />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-3xl border border-dash-border/60 bg-white p-6 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <div className="mb-5 flex items-center justify-between">
            ...
            <Link href="/admin/reports" className="flex items-center gap-1 text-xs font-bold text-dash-brand hover:underline">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                },
                {
                  ...
                  sub: <span className="text-dash-muted">countries active</span>,
                  status: "ready" as const,
                },
              ].map((card) => (
                <div key={card.label} className="flex flex-col justify-between rounded-xl border border-dash-border p-4">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-xs font-bold leading-tight text-dash-text">{card.label}</span>
                  ...
                  <p className="text-2xl font-black text-dash-brand">{card.value}</p>
          ...
          <div className="flex divide-x divide-dash-border rounded-2xl border border-dash-border bg-white">
            {[
              ...
              <div key={label} className="flex flex-1 flex-col items-center py-4">
                ...
                <span className="mt-0.5 text-[10px] font-semibold text-dash-muted">{label}</span>
              </div>
            ))}
          </div>

          <div className="flex-1 rounded-3xl border border-dash-border/60 bg-white p-5 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
            <h2 className="mb-4 text-sm font-black text-dash-text">Recent Exports</h2>
            {!snapshot || snapshot.recent_exports.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <p className="text-sm text-dash-muted">No exports yet.</p>
                <p className="mt-1 text-xs text-dash-subtle">Run a report export and it appears here.</p>
              </div>
            ) : (
              <ul className="space-y-3">
                {snapshot.recent_exports.map((ex) => (
                  <li key={ex.id} className="flex items-center justify-between gap-2 rounded-xl border border-dash-border p-3">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-3xl border border-dash-border/60 bg-white p-6 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-dash-text">Pending Supplier Approvals</h2>
              <p className="text-xs font-semibold text-dash-muted">{pendingSuppliers.length} waiting for review</p>
            </div>
          </div>
          {loading ? (
        ...
        <div className="rounded-3xl border border-dash-border/60 bg-white p-6 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-dash-text">Pending Agent Approvals</h2>
              <p className="text-xs font-semibold text-dash-muted">{pendingAgents.length} waiting for review</p>
            </div>
          </div>
          {loading ? (
            <div className="h-16 animate-pulse rounded-xl bg-slate-100" />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-3xl border border-dash-border/60 bg-white p-6 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
          <div className="mb-4">
            <h2 className="text-base font-black text-dash-text">Quick Actions</h2>
            <p className="mt-0.5 text-xs font-semibold text-dash-muted">Jump to common admin tasks</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              >
                ...
                <p className="mt-1 text-[11px] font-semibold leading-snug text-dash-muted">{act.desc}</p>
              </Link>
            ))}
          </div>
        </div>
        ...
        {/* Activity + alerts */}
        <div className="flex flex-col gap-4">
          <div className="rounded-3xl border border-dash-border/60 bg-white p-5 shadow-[0_2px_12px_rgb(0,0,0,0.03)]">
  ...
  if (!dashboard) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-dash-bg">
        ...
        <button type="button" onClick={() => void refetch()} className="ml-3 text-sm font-bold text-dash-brand">
          Retry
        </button>
      </div>
    );
