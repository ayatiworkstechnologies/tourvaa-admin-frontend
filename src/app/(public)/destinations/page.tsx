    ...
    <main className="min-h-screen bg-slate-50 pb-20">
      ...
      <section className="relative z-10 mx-auto -mt-8 max-w-4xl px-5 md:px-8">
        <div className="rounded-2xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-900/10">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-pub-secondary" size={20} />
            <input
              type="search"
              ...
              className="min-h-14 w-full rounded-xl bg-slate-50 py-4 pl-12 pr-24 text-sm font-bold text-slate-950 outline-none transition focus:bg-white focus:ring-2 focus:ring-pub-secondary/20"
            />
            {search && (
      ...
      <section className="mx-auto mt-14 max-w-7xl px-5 md:px-8">
        <div className="mb-7 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          ...
          {!loading && <p className="text-sm font-bold text-slate-500"><span className="text-pub-secondary">{filteredCountries.length}</span> destination{filteredCountries.length === 1 ? "" : "s"}</p>}
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-20 text-center">
            ...
            <button type="button" onClick={() => setSearch("")} className="mt-6 rounded-xl bg-pub-secondary px-6 py-3 text-sm font-black text-white transition hover:bg-pub-secondary/90">View all destinations</button>
          </div>
        )}
      </section>
