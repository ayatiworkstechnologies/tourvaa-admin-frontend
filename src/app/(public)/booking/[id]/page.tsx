  ...
  const createBooking = handleSubmit(async (form) => {
    ...
    try {
      ...
      const response = await api.post("/customer/bookings", {
        ...
        no_of_adults: form.adults, no_of_children: form.children, currency: tour.currency || "AED", booking_source: "customer", payment_type: form.paymentType,
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-7">
        ...
        <form onSubmit={createBooking} className="mx-auto grid max-w-5xl gap-5 lg:grid-cols-[minmax(0,1fr)_310px]">
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
            ...
            {step === 5 && booking && <div><Header n="5" title="Secure Payment" text="Choose a gateway to complete your payment." /><div className="rounded-2xl bg-[#063c42] p-6 text-white"><p className="text-xs font-bold uppercase tracking-widest text-teal-200">Pay securely now</p><p className="mt-2 text-4xl font-black">{money(paymentAmount, booking.currency)}</p><p className="mt-2 text-sm text-white/70">Booking {booking.booking_code} · {values.paymentType === "partial" ? "30% deposit" : "Full payment"}</p></div><div className="mt-5 space-y-3"><PayButton disabled={!gateways?.stripe || !!busy} onClick={() => pay("stripe")} loading={busy === "stripe"} label="Pay with Stripe" /><PayButton disabled={!gateways?.paypal || !!busy} onClick={() => pay("paypal")} loading={busy === "paypal"} label="Pay with PayPal" />{gateways?.test_mode_available && <PayButton disabled={!!busy} onClick={() => pay("test")} loading={busy === "test"} label="Simulate test payment" test />}</div><div className="mt-5 flex items-start gap-3 rounded-2xl border border-teal-100 bg-teal-50 p-4 text-sm text-teal-800"><ShieldCheck className="shrink-0" size={20} /><span>Your payment is encrypted. Booking confirmation remains subject to supplier acceptance.</span></div></div>}
