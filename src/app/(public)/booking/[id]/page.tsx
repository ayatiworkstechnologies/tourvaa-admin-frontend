...
export default function PublicBookingPage() {
  ...
  if (isLoggedIn && !isCustomer && !isAgent) return <div className="mx-auto max-w-lg px-5 py-28 text-center"><Lock className="mx-auto text-teal-700" size={42} /><h1 className="mt-5 text-3xl font-black text-slate-950">Sign in to continue booking</h1><p className="mt-3 text-slate-500">Customers and agents can use this same booking journey.</p><div className="mt-7 flex flex-wrap justify-center gap-3"><Link href={`/login?role=traveller&redirect=${encodeURIComponent(`/booking/${tour.id}?${searchParams.toString()}`)}`} className="inline-flex rounded-xl bg-teal-700 px-6 py-3.5 font-bold text-white">Customer login</Link><Link href={`/login?role=agent&redirect=${encodeURIComponent(`/booking/${tour.id}?${searchParams.toString()}`)}`} className="inline-flex rounded-xl border border-teal-700 px-6 py-3.5 font-bold text-teal-800">Agent login</Link></div></div>;
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-7">
        ...
        <form onSubmit={createBooking} className="mx-auto grid max-w-5xl gap-5 lg:grid-cols-[minmax(0,1fr)_310px]">
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
                    ...
                    return (
                      <div key={field.id} className="rounded-2xl border border-slate-200 p-4">
                        ...
                        <div className="mt-3 grid gap-3 sm:grid-cols-3">
                          ...
                          <label className="field-label">Date of birth<input type="date" {...register(`travellers.${index}.date_of_birth`)} className="field" /></label>
                        ...
                        {!isPrimary && (
                          <div className="mt-3 grid gap-3 sm:grid-cols-2">
                            ...
                            <label className="field-label">Passport expiry <span className="font-normal normal-case text-slate-400">(optional)</span><input type="date" {...register(`travellers.${index}.passport_expiry_date`)} className="field" /></label>
                          </div>
                        )}
                      </div>
                    );
            ...
            {step === 5 && booking && <div><Header n="5" title="Secure Payment" text="Choose a gateway to complete your payment." /><div className="rounded-2xl bg-[#063c42] p-6 text-white"><p className="text-xs font-bold uppercase tracking-widest text-teal-200">Pay securely now</p><p className="mt-2 text-4xl font-black">{formatCurrency(paymentAmount, booking.currency)}</p>{displayCurrency !== booking.currency && <p className="mt-1 text-xs font-semibold text-teal-200">≈ {money(paymentAmount, booking.currency)} in your selected currency</p>}<p className="mt-2 text-sm text-white/70">Booking {booking.booking_code} · {values.paymentType === "partial" ? "Deposit payment" : "Full payment"}</p></div><div className="mt-5 space-y-3"><PayButton disabled={!gateways?.stripe || !!busy} onClick={() => pay("stripe")} loading={busy === "stripe"} label="Pay with Stripe" /><PayButton disabled={!gateways?.paypal || !!busy} onClick={() => pay("paypal")} loading={busy === "paypal"} label="Pay with PayPal" />{gateways?.test_mode_available && <PayButton disabled={!!busy} onClick={() => pay("test")} loading={busy === "test"} label="Simulate test payment" test />}</div><div className="mt-5 flex items-start gap-3 rounded-2xl border border-teal-100 bg-teal-50 p-4 text-sm text-teal-800"><ShieldCheck className="shrink-0" size={20} /><span>Your payment is encrypted. Booking confirmation remains subject to supplier acceptance.</span></div></div>}
  ...
  return (
    <div>
      ...
      {phase === "email" ? (
        ...
        <div className="space-y-4">
          <label className="field-label">Verification code *<input type="text" inputMode="numeric" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ""))} onKeyDown={(event) => event.key === "Enter" && (event.preventDefault(), verifyOtp())} placeholder="123456" className="field tracking-[0.4em]" /></label>
