...
function Panel({ title, icon: Icon, iconColor = "text-dash-brand", iconBg = "bg-[#F0F7FF]", children }: { title: string; icon?: React.ComponentType<{ size?: number; className?: string }>; iconColor?: string; iconBg?: string; children: React.ReactNode }) {
  return (
...
function PayNowModal({
  ...
  async function payWithTest() {
    ...
    try {
      await api.post("/payments/test/simulate", { booking_id: bookingId, amount, note: "Test payment from UI" });
      onClose();
      onSuccess();
    } catch (e: unknown) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-3xl border border-dash-border bg-white p-6 shadow-2xl">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-black text-dash-text">Complete Payment</h3>
            <p className="text-sm text-dash-muted">
              Amount due: <strong className="text-dash-text">{currency} {amount.toLocaleString()}</strong>
            </p>
          </div>
      ...
      if (savedPid) {
        ...
        api.post("/payments/paypal/capture", { order_id: token, payment_id: parseInt(savedPid) })
          .then(() => {
            ...
            setPaymentBanner({ type: "success", msg: "PayPal payment captured! Your booking is confirmed." });
            void load();
          })
          .catch(() => {
                    ...
                    <div key={t.id} className="flex flex-wrap items-start justify-between gap-2 rounded-xl border border-dash-border px-4 py-3">
                      <div>
                        <p className="font-semibold text-dash-text">
                          ...
                          {t.is_primary_contact && <span className="ml-2 rounded-full bg-[#F0F7FF] px-2 py-0.5 text-[10px] font-bold text-dash-brand">Primary</span>}
                        </p>
      ...
      {showPayModal && booking && (
        <PayNowModal
          ...
          onSuccess={() => {
            ...
            setPaymentBanner({ type: "success", msg: "Test payment successful! Booking is now confirmed." });
            void load();
          }}
        />
      )}
