...
import { ChatMessage, ConversationThread, ParticipantType, getOwnConversation, sendOwnMessage } from "@/lib/api/services/messagingService";
        ...
        {loading ? (
          ...
          thread?.messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender_role === "admin" ? "justify-start" : "justify-end"}`}>
              <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${msg.sender_role === "admin" ? "bg-dash-bg text-dash-text" : "bg-dash-brand text-white"}`}>
