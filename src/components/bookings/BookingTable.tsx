...
export default function BookingTable({ onCancel, onConfirm, ...tableProps }: BookingTableProps) {
  const columns: DataTableColumn<Booking>[] = [
    { key: "booking_code", header: "Booking", className: "font-bold text-dash-text" },
    {
      ...
      render: (booking) => (
        <div>
          <p className="font-semibold text-dash-text">{booking.customer_name || `Customer #${booking.customer_id}`}</p>
          {booking.customer_email && <p className="text-xs text-dash-subtle">{booking.customer_email}</p>}
        </div>
      ),
    },
    {
      ...
      render: (booking) => (
        <div>
          <p className="font-semibold text-dash-body">{booking.tour_name}</p>
          {booking.tour_date && <p className="text-xs text-dash-subtle">{booking.tour_date}</p>}
        </div>
      ),
    },
    {
