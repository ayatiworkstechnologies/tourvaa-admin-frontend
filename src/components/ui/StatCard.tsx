...
export default function StatCard({
  ...
  return (
    <div className="flex min-h-[84px] items-center gap-4 rounded-xl bg-[#E7F5FF] px-4 py-4">
      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-white text-dash-brand">
        {Icon ? <Icon size={26} strokeWidth={1.9} /> : null}
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium text-[#7A8391]">{title}</p>
        <h3 className="mt-1 text-3xl font-bold leading-none text-dash-text">
          {value}
        </h3>
      </div>
      <span
