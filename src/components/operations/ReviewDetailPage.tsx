...
import { approveReviewRecord, getReviewRecord, partialApproveReviewRecord, rejectReviewRecord, ReviewModule, ReviewRecord, updateAffiliateApiLink, updateCommercialValue } from "@/lib/api/services/operationsService";
...
function DetailSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="rounded-xl border border-dash-border bg-white p-6">
      <h3 className="text-lg font-bold text-dash-text">{title}</h3>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <Link href={`/admin/${module}`} className="inline-flex items-center gap-2 text-sm font-bold text-dash-brand-hover">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {fieldRows(record).map(([label, value]) => (
                <div key={label} className="rounded-lg bg-dash-bg p-4">
                  ...
                  <div className="mt-1 text-sm font-semibold text-dash-text">{label === "Status" || label === "Approval" ? <StatusBadge value={String(value || "")} /> : value || "-"}</div>
                </div>
              ))}
            </div>
          </DetailSection>
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            <DetailSection title="Business Information">
              <pre className="overflow-auto rounded-lg bg-dash-bg p-4 text-xs text-dash-body">{JSON.stringify(record.business_info || record.marketing_info || {}, null, 2)}</pre>
            </DetailSection>
            <DetailSection title="Invoicing">
              <pre className="overflow-auto rounded-lg bg-dash-bg p-4 text-xs text-dash-body">{JSON.stringify(record.invoicing || {}, null, 2)}</pre>
            </DetailSection>
          </div>

          <DetailSection title="Documents">
            <pre className="overflow-auto rounded-lg bg-dash-bg p-4 text-xs text-dash-body">{JSON.stringify(record.documents || [], null, 2)}</pre>
          </DetailSection>
        ...
        <section className="rounded-xl border border-dash-border bg-white p-10 text-center text-dash-muted">Record not found.</section>
      )}
    </ModuleWrapper>
  );
}
