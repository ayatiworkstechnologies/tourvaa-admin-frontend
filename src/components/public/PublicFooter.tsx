...
function normalizeFooterSections(sections: CmsFooterSection[]): CmsFooterSection[] {
  return sections.map((section) => {
    const isPartnerCol =
      section.title.toLowerCase() === "login" ||
      section.title.toLowerCase().includes("portal") ||
      section.title.toLowerCase().includes("partner");
    ...
    if (isPartnerCol) {
      return {
        ...
        links: section.links.map((link) => {
          ...
          if (lowerLabel.includes("supplier") || lowerUrl.includes("supplier-portal")) {
            return {
              ...link,
              label: "Supplier Portal",
              url: "/supplier-portal",
          ...
          if (lowerLabel.includes("agent") || lowerUrl.includes("agent-portal")) {
            return {
              ...link,
              label: "Agent Portal",
              url: "/agent-portal",
          ...
          if (lowerLabel.includes("affiliate") || lowerUrl.includes("affiliate-portal")) {
            return {
              ...link,
              label: "Affiliate Programme",
              url: "/affiliate-portal",
          ...
          if (lowerLabel.includes("traveller") || lowerLabel.includes("traveler") || lowerUrl === "/login") {
            return {
              ...link,
              label: "Traveller Sign In",
              url: "/login",
...
export default function PublicFooter() {
    ...
    fetchFooterSections()
      .then((sections) => {
        if (active && sections.length > 0) {
          setFooterSections(normalizeFooterSections(sections));
        }
      })
      .catch(() => {
        /* Fixed fallback list remains available. */
  ...
  return (
    <footer className="bg-white text-slate-700 pt-1 pb-6 sm:pb-8">
      {/* Directory Section (Top countries to visit / Top Cities to Visit / Top attraction categories) */}
      <div className="mx-auto max-w-[1400px] px-3 sm:px-6 lg:px-8 mb-5 sm:mb-6">
                          ...
                          {countryOptions.map((item) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setCountryOpen(false);
                                router.push(
                                  `/tours?country=${encodeURIComponent(item.name)}`,
                                );
                              }}
                              className={`flex w-full items-center justify-between rounded-lg px-2.5 py-1.5 text-xs font-semibold transition ${
