...
const THEME_CLASSES: Record<
  PortalTheme,
  { bg: string; border: string; accent: string; button: string }
> = {
  emerald: {
    bg: "bg-emerald-950",
...
export default function PortalPublicHeader({
  ...
  return (
    <header
      className={`sticky top-0 z-40 border-b ${classes.border} ${classes.bg} text-white`}
    >
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
        ...
        >
          ...
          Tourvaa{" "}
          <span className={`font-semibold ${classes.accent}`}>{roleLabel}</span>
        </Link>
        <nav className="flex items-center gap-3 text-sm font-bold">
          ...
          <Link
            ...
            className={`rounded-xl bg-white px-4 py-2  transition hover:-translate-y-0.5  ${classes.button}`}
          >
            Login / Register
          </Link>
          <Link
            href="/"
            className="hidden text-xs font-semibold text-white/50 transition hover:text-white/80 sm:inline"
          >
            Main site
          </Link>
        </nav>
